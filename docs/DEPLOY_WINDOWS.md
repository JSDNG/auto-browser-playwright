...
- **Chrome**:
  - Phải khởi động với `--remote-debugging-port=9224`.
- **Domain ngoài**:
...
API `check_delivery_status` yêu cầu Chrome chạy với cờ `--remote-debugging-port=9224`:
...
3. Khởi động Chrome với `--remote-debugging-port=9224`.
