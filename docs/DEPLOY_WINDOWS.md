...
- **Chrome**:
  - Sẽ được tự động launch bởi API (không cần khởi động trước)
  - Hoặc có thể khởi động với `--remote-debugging-port=9224` nếu dùng CLI với CDP connection
- **Domain ngoài**:
...
## 4. Khởi động Chrome với CDP (remote debugging) - Tùy chọn

**Lưu ý:** Chỉ cần thiết nếu bạn sử dụng CLI với CDP connection:
- CLI: `python src/app/cdp_connection.py "prompt text"`

**Không cần thiết** nếu chỉ dùng API endpoint `/api/v1/grok/launch` (tự động launch Chrome).
...
- Domain: `grok-api.yourdomain.com` (ví dụ).
...
- Tạo bản ghi:
  - **Type**: `A`
  - **Name**: `grok-api.yourdomain.com` (ví dụ)
...
#### Bước reverse proxy (Nginx trên Windows, cấu hình tách file riêng `grok_api.conf`)
...
- Mỗi project tạo **một file riêng** trong `conf.d`, ví dụ:
  - `C:\nginx\conf\conf.d\grok_api.conf`

1. Cài Nginx cho Windows trên chính máy `192.168.1.94` (ví dụ tại `C:\nginx`).
2. Tạo thư mục `C:\nginx\conf\conf.d\` nếu chưa có.
3. Tạo file mới: `C:\nginx\conf\conf.d\grok_api.conf` với nội dung:

```nginx
server {
    listen 80;
    server_name grok-api.yourdomain.com;
...
```bash
nslookup grok-api.yourdomain.com
```
  - Hoặc:
```bash
ping grok-api.yourdomain.com
```
...
5. Kiểm tra:
   - Nếu domain provider đang terminate SSL (ví dụ Cloudflare Full/Proxied):
     - Truy cập `https://grok-api.yourdomain.com/docs` từ ngoài Internet.
   - Nếu chỉ dùng HTTP nội bộ:
     - Truy cập `http://grok-api.yourdomain.com/docs` trong mạng tương ứng.
...
- `http://127.0.0.1:5674/docs` trên server:
  ...
  - Swagger UI sẽ hiển thị endpoint: Grok Video Generation
...
Xem `docs/API_GUIDE.md` để biết chi tiết về Grok video generation endpoint.
...
- **Grok endpoint:**
  ...
  - Timeout nên đặt khoảng 60 giây vì cần thời gian launch browser và navigate.
  - Có thể tối ưu thời gian chờ (hiện tại chờ 5 giây sau navigate, 2 giây sau click).

- **Nếu lượng request lớn:**
  - Xem xét chạy nhiều instance service trên các port khác nhau rồi load balance.
  - Hoặc tối ưu logic trong `grok_gen_video_direct` và `_grok_imagine_interact`.
...
3. **Chuẩn bị browser (nếu cần):**
   - **CLI với CDP:** Khởi động Chrome với `--remote-debugging-port=9224`
   - **API endpoint:** Không cần, tự động launch
...
6. **Kiểm tra:**
   ...
   - Test endpoint: Grok Video Generation
