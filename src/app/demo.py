"""
Demo script để launch Playwright với HideMyAcc profile qua user-data-dir
Demo mở trang Etsy và giữ browser mở
"""
import asyncio
import sys
...
async def demo_launch_with_profile(profile_id: str = None, use_command_line_config: bool = False, disable_proxy: bool = False):
    """
    Demo launch Playwright với HideMyAcc profile và mở trang Etsy
    
    Args:
        profile_id: Profile ID để sử dụng
        use_command_line_config: Nếu True, sử dụng cấu hình từ command line (proxy, args, executable)
        disable_proxy: Nếu True, tắt proxy
    """
    print("=" * 60)
    print("Demo: Launch Playwright với HideMyAcc Profile")
    print("=" * 60)
    print()
    ...
    if not profile_id:
        ...
        if len(profiles) > 1:
            ...
            print("   Ví dụ: python3 src/app/demo.py --profile hma_xxx")
            return None
        else:
            profile = profiles[0]
    ...
    if use_command_line_config:
        ...
        # Tìm Marco browser từ HideMyAcc
        executable_path = manager._find_marco_browser()
        proxy = None
        extra_args = None
    ...
    try:
        ...
        if proxy and not disable_proxy:
            ...
            try:
                test_proxy_url = "https://httpbin.org/ip"
        ...
        try:
            ...
            print("  python3 src/app/demo.py --profile 6898af88effa52a76ecbe4ec --use-command-line --no-proxy")
            print()
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Demo: Launch Playwright với HideMyAcc profile và mở trang Etsy")
    ...
    await demo_launch_with_profile(
        args.profile, 
